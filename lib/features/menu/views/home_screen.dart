    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('¡Camarero llamado!')),
        );
      }
    } catch (e) {
      if (mounted) {
    ...
    return Scaffold(
      ...
      body: _selectedTab == 0
        ...
        children: [
          ...
          // Lista de platos con fade y fly-to-cart
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: KeyedSubtree(
                key: ValueKey(
                    '${_categoriaSeleccionada.id}|$_searchQuery'),
                child: platosAsync.when(
                  loading: () =>
                  const Center(child: CircularProgressIndicator()),
                  error: (e, _) => Center(child: Text('Error: $e')),
                  data: (platos) {
                    final list = platos.where((p) {
                      final matchCat =
